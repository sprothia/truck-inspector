...
This builds the app, syncs to the iOS project, and opens Xcode. In Xcode, select your iPhone, sign with your Apple ID, and run.

**On device:** The proxy URL defaults to `localhost`, which won't work on a physical iPhone. Run `server.py` on your Mac, find your Mac's local IP (e.g. `192.168.1.x`), and set the proxy URL in the app to `ws://YOUR_MAC_IP:8080`.
